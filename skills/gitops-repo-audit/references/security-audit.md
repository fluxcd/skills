...
- [ ] **No cross-namespace source refs**: In multi-tenant setups, check for `sourceRef.namespace` where a Kustomization or HelmRelease in one namespace references a source in another namespace. This bypasses tenant isolation. Enable `--no-cross-namespace-refs=true` on controllers (via FluxInstance `multitenant: true` or kustomize patches). Exception: a platform-team monorepo that runs with `multitenant: false` and generates app Kustomizations from ResourceSets deliberately references `ExternalArtifact`/`OCIRepository` sources in `flux-system` — report that as Info (single trust domain), not as a Critical isolation break, unless the repo also hosts untrusted tenants
- [ ] **ExternalArtifact providers scoped to tenants**: A `ResourceSetInputProvider` of `type: ExternalArtifact` lists in-cluster artifacts with the operator's own (cluster-wide) service account unless `spec.serviceAccountName` is set. In multi-tenant clusters each tenant's provider should impersonate the tenant SA; a `selectors[].namespace: "*"` selector then requires that SA to hold cluster-wide `list` on ExternalArtifacts — flag providers that discover across all namespaces without a scoped service account
...
Search for `sourceRef:` blocks where `namespace:` differs from the resource's own namespace. Look for `.spec.sourceRef.namespace` and `.spec.chartRef.namespace` fields, including inside ResourceSet templates. Check the FluxInstance `cluster.multitenant` value before deciding severity (see the checklist item above).
