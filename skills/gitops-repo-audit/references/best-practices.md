...
- [ ] **No `${var}` in `spec.images[].name`**: Kustomize image rewriting runs at build time, before post-build substitution, so a `Kustomization.spec.images[].name` (or a `kustomization.yaml` `images[].name`) containing `${...}` never matches anything. The `name` must be the plain image reference used in the manifests (`image: frontend`); `newName`/`newTag` may use variables
